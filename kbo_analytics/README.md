...
| `modeling/results/pregame_prediction_history.csv` | 경기 전 예측 실행 이력과 직전 실행 대비 확률 변화 |
...
경기 전 업데이트가 반복 실행될 때마다 오늘 경기 예측을 이력으로 저장합니다.

저장 파일:

| 파일 | 내용 |
| --- | --- |
| `modeling/results/pregame_prediction_history.csv` | 실행 시각, 경기 ID, 예측 구단, 예측승률, 선발 상태, 라인업 상태, 직전 실행 대비 변화 |

이 파일을 사용해 대시보드 예측 카드에 아래 내용을 표시합니다.

- 이전 예측 없음
- 직전 예측 대비 변화 거의 없음
- 직전 예측 대비 +2.1%p
- 직전 예측 대비 우세 강화 +3.4%p
- 직전 예측 대비 우세 약화 -3.2%p
- 예측 구단 변경

이 기능의 목적은 확정 선발이나 라인업 상태가 바뀌었을 때 승률이 얼마나 움직였는지 추적하는 것입니다.
...
구단 분석 리포트와 기본 승패 예측 모델에 더해, 선수 능력치를 시간 기준으로 결합하는 독립 shadow challenger가 구현되어 있습니다. 선수 데이터 coverage와 leakage Gate를 통과하기 전에는 운영 모델을 교체하지 않습니다.

운영 자동화의 실행 계약, 안전 기본값, systemd 템플릿과 rollback 절차는 [AUTOMATION.md](AUTOMATION.md)를 참고하세요.
선수 challenger의 시간 기준과 검증 정책은 [PLAYER_CHALLENGER.md](PLAYER_CHALLENGER.md)를 참고하세요.
